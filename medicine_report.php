...
<head>
    ...
    <title>SAVANNAH HOSPITAL | Medicine List</title>
    <style>
        header {
            display: flex;
            align-items: center;
...
<body>
    ...
    <form method="GET">
        <div>
            <label for="searchMedicineId">Search by Medicine Id:</label>
            <input type="text" id="searchMedicineId" name="searchMedicineId">
        </div>
      <div>
    ...
    <table>
        ...
        $sql = "SELECT * FROM Medicine_table WHERE 1=1"; 
        if (isset($_GET['searchMedicineId']) && !empty($_GET['searchMedicineId'])) {
                $clean_searchMedicineId = $_GET['searchMedicineId'];
                    $sql .= " AND Med_id = $clean_searchMedicineId";
        ...
        if (isset($_GET['month']) && !empty($_GET['month']) && isset($_GET['year']) && !empty($_GET['year']) && validateInteger($_GET['month']) && validateInteger($_GET['year'])) {
            $month = $_GET['month'];
        $year = $_GET['year'];
        ...
        if (isset($_GET['monthRangeFrom']) && isset($_GET['yearRangeFrom']) && isset($_GET['monthRangeTo']) && isset($_GET['yearRangeTo']) &&
            !empty($_GET['monthRangeFrom']) && !empty($_GET['yearRangeFrom']) && !empty($_GET['monthRangeTo']) && !empty($_GET['yearRangeTo'])){ $monthRangeFrom = $_GET['monthRangeFrom'];
        ...
        if (strpos($sql, 'WHERE') !== false) {
            ...
            echo "<tr><td colspan='8'>Please provide at least one search criteria.</td></tr>";
        }
    ...
    function printOrdersReport() {
        ...
        var printableContent = "<html><head><title>Savannah Hospital Medicine Report</title></head><body>";
        printableContent += "<img src='../Savannah-logo.png' alt='Hospital Logo'>";
        printableContent += "<h2>Savannah Hospital Medicine Report</h2>";
