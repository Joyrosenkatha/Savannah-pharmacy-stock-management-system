...
// Redirect the user form the current page to the Edit_patientList page with the Patient_id parameter in the query string that handles updating
        window.location.href = 'editBooking_receptionist.php?Booking_id=' + BookingId;
    }
</script>
...
<nav>
            ...
            <div class="Medicine-h1">
                       <h1>SAVANNAH- <span class="danger">BOOKING INFORMATION</span></h1>
                    </div>
                    <br>
...
<form method="GET" action="">
    <input type="text" name="search_query" placeholder="Search by patient id, Booking status">
...
if(isset($_GET['search'])) {
  ...
  $sql = "SELECT * FROM patient_booking WHERE Booking_status LIKE '%$search_query%' OR Patient_id LIKE '%$search_query%'";
  ...
  if($result->num_rows > 0) {
    //fetch_assoc()` fetches the next row from the result set returned by the executed query and returns it as an associative array.
      while ($row = $result->fetch_assoc()) {
        ...
        $result = $link->query("SELECT * FROM patient_booking");
        while ($row = $result->fetch_assoc()) {
            echo "<tr>";
            echo "<td>" . $row["Booking_id"] . "</td>";
            echo "<td>" . $row["Doc_id"] . "</td>";
