<D html>
 ...
 <header>
    ...
    <div>
        ...
        <br>
        <br>
        <button><a href="Doctor_dashboard.php">Go Back</a></button>
    </div>
...
if (isset($_GET['startDate']) && !empty($_GET['startDate']) && isset($_GET['endDate']) && !empty($_GET['endDate'])) {
    ...
    if ($endDate >= $startDate) {
        ...
        echo "<p><span style='color: red;'>End Date cannot be before Start Date.</span></p>";
    }
}
    ...
    if (strpos($sql, 'WHERE') !== false) {
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                ...
                echo "<tr><td colspan='7'>No Prescription found.</td></tr>";
            }
        } else {
            echo "Error: " . $link->error;
        }
    } else {
        echo "<tr><td colspan='7'>Please provide at least one search criteria.</td></tr>";
    }
    $link->close();
    ?>
    </tbody>
...
function printOrdersReport() {
    var printableContent = "<html><head><title>Savannah Hospital Prescription Report</title></head><body>";
    printableContent += "<img src='../Savannah-logo.png' alt='Hospital Logo'>";
    printableContent += "<h2>Savannah Hospital Prescription Report</h2>";
